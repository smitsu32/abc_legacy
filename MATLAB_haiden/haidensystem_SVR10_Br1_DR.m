...
nEss=2;% (nNode-sum(TF))*SpecStruct.Ess.nEssPerNode;
...
TimeInfo = MyProgram.Function.simTimeSet(Main.DayLoop.startTime,min(Main.DayLoop.endTime,Main.DayLoop.startTime+days(1)),minutes(2),...
...
% SvrTapは10分値で CはカウントのC
        ...
        SvrTap = optimvar ('SvrTap',Opt.nTime/5,nSvr,1,...
                'Type','integer','LowerBound',-(Svr.nTap-Svr.defTap).*ones(Opt.nTime/5,nSvr),'UpperBound',(Svr.nTap-Svr.defTap).*ones(Opt.nTime/5,nSvr));
%(Svr.nTap-Svr.defTap).*ones
        SvrTapC = optimvar ('SvrTapC',Opt.nTime/5,nSvr,1,...
                'Type','integer','LowerBound',zeros(Opt.nTime/5,nSvr),'UpperBound',ones(Opt.nTime/5,nSvr));
% LRT動作(LRT):整数,素通しが0
...
% LrtTapは10分値で
        ...
        %
        LrtTap = optimvar ('LrtTap',Opt.nTime/5,nLrt,1,...
                'Type','integer','LowerBound',ones(Opt.nTime/5,nLrt),'UpperBound',Lrt.nTap.*ones(Opt.nTime/5,nLrt));

        LrtTapC = optimvar ('LrtTapC',Opt.nTime/5,nLrt,1,...
                'Type','integer','LowerBound',zeros(Opt.nTime/5,nLrt),'UpperBound',ones(Opt.nTime/5,nLrt));
        %}
% 
%% 目的関数
...
for iLRT=1:nLrt    
    ...
    LRTVolRe(1:Opt.nTime,iLRT,1:Opt.nPhase) = 1.0*nodeV_Re(1:Opt.nTime,Lrt.node(iLRT),:) ...
        - repmat(repelem(Lrt.dVTap/sqrt(3)*LrtTap(:,iLRT),5),1,1,Opt.nPhase);
...
SVRV=optimexpr(Opt.nTime/5,nNode);%
% 影響を与えるノード
%
 for iSVR=1:nSvr
        for iTime=1:Opt.nTime/5
            SVRV(iTime,1:nNode,:)=SVRV(iTime,1:nNode,:)...
                +(Svr.dVTap(iSVR)/sqrt(3)*SvrTap(iTime,iSVR,:).*Svr.effNode(:,iSVR,:)');
               % +(Svr.dVTap(iSVR)/sqrt(3)*(SvrTap(iTime,iSVR,:)-4).*Svr.effNode(:,iSVR,:)');
        end
...
for iCha=1:nCharger
    ...
    type=find(Charger.phase(iCha)==phase);
    if isempty(type)
        type=1:length(phase);
    end
...
prob.Constraints=MyProgram.OptFunction.SLimit(prob.Constraints,"nodeVMax",5,nodeV_Re,nodeV_ImAbs,nodeV);
...
prob.Constraints.SVR1NTapC = sum(SvrTapC(:,1),1) <=50;
...
%以下、初期化
    for iTime=Opt.nTime*1:-1:1 %時間も変更
        qc(iTime).a = sparse(xNum+Opt.nTime,1);
...
%dataTT(datetime(2017,11,9):minutes(2):datetime(2017,11,9,23,58,0),:)=[]; %データ欠損
Time=dataTT.Time;
...
for iField=1:length(fieldNames)
    netdemTT.(string(fieldNames(iField)))=zeros(length(Time),iNode);
end
...
%                 else
                    ...
                    SVRTAP(:,1)=repelem(x0(xVar.SvrTap(1:144),1),5,1);%x0の8740～8787番目がSvr1、8788～8835番目がSvr2
                    SVRTAP(:,2)=repelem(x0(xVar.SvrTap(145:288),1),5,1);
                    tmp_TT.SvrTap=SVRTAP;
                    tmp_TT.LrtTap=repelem(x0(xVar.LrtTap(1:144),1),5,1);
            % end
            else
